...
resource "aws_appautoscaling_target" "ecs" {
  ...
  resource_id        = "service/${aws_ecs_cluster.jobboard.name}/${aws_ecs_service.jobboard[count.index].name}"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  count               = local.create_ecs_service
  alarm_name          = "ecs_cpu_utilization_high_jb"
  ...
  dimensions = {
    ClusterName = aws_ecs_cluster.jobboard.name
    ServiceName = aws_ecs_service.jobboard[count.index].name
  }
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_low" {
  count               = local.create_ecs_service
  alarm_name          = "ecs_cpu_utilization_low_jb"
