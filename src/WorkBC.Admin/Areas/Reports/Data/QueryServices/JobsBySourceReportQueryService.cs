    ...
    public class JobsBySourceReportQueryService : QueryServiceBase
    {
        ...
        public async Task<IList<JobStatsResult>> RunYearlyAsync(int startYear, int endYear)
        {
            ...
            var sql =
                @"WITH dimensions AS 
                    (
                        SELECT DISTINCT js.Id AS JobSourceId, js.GroupName AS Label, wp.FiscalYear
                        FROM WeeklyPeriods wp, JobSources js
                    ...
                    (
	                    ...
	                    GROUP BY jr.JobSourceId, wp.FiscalYear
                    )
                    SELECT d.Label, 
	                    d.FiscalYear,
        ...
        public async Task<IList<JobStatsResult>> RunMonthlyAsync(DateTime startDate, DateTime endDate)
        {
            ...
            var sql =
                @"WITH dimensions AS 
                    (
	                    SELECT DISTINCT js.Id AS JobSourceId, js.GroupName AS Label, 
                            wp.CalendarMonth, wp.CalendarYear
	                    FROM WeeklyPeriods wp, JobSources js
                    ...
                    (
	                    ...
	                    FROM JobStats jr
		                    INNER JOIN weeklyPeriods wp ON wp.Id = jr.WeeklyPeriodId
	                    GROUP BY jr.JobSourceId, wp.CalendarMonth, wp.CalendarYear
                    )
                    SELECT d.Label, 
	                    d.CalendarMonth, 
	                    d.CalendarYear,
            ...
            var sql =
                @"WITH dimensions AS 
                    ...
                    (
	                    ...
	                    FROM JobStats jr
		                    INNER JOIN WeeklyPeriods wp ON wp.Id = jr.WeeklyPeriodId
	                    GROUP BY jr.JobSourceId, jr.WeeklyPeriodId
                    )
                    SELECT d.Label, 
	                    d.WeekOfMonth,
                        d.CalendarMonth,
